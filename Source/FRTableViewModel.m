...
@interface FRTableViewDelegate : NSObject <UITableViewDelegate>

+ (instancetype) new NS_UNAVAILABLE;
- (instancetype)init NS_UNAVAILABLE;

- (instancetype)initWithViewModel:(__kindof FRTableViewModel *)viewModel;

@property (nullable, nonatomic, weak, readonly) __kindof FRTableViewModel *viewModel;

@end

@interface FRTableViewDataSource : NSObject <UITableViewDataSource>
...
@property (nonatomic, strong) NSMutableArray<__kindof id<FRTableSectionViewModelProtocol>> *sections;

@property (nonatomic, strong) FRTableViewFactory *tableViewFactory;
...
FRTableSectionViewModel *section = [FRTableSectionViewModel section];     \
...
@implementation FRTableViewModel{
...
- (instancetype)init {
    if (self = [super init]) {
        _delegate = [[FRTableViewDelegate alloc] initWithViewModel:self];
        _dataSource = [[FRTableViewDataSource alloc] initWithViewModel:self];
        self.tableViewFactory = [[FRTableViewFactory alloc] init];
    }
    return self;
}
...
- (NSInteger)sectionIndexForHeaderViewModel:(__kindof id<FRTableItemViewModelProtocol>)headerViewModel {
    ...
    for (NSInteger i = 0; i < self.sections.count; i++) {
        id<FRTableSectionViewModelProtocol> sectionViewModel = self.sections[i];
...
- (nullable __kindof id<FRTableItemViewModelProtocol>)headerViewModelFromSection:(NSInteger)sectionIndex {
    ...
    id<FRTableSectionViewModelProtocol> sectionViewModel = self.sections[sectionIndex];
    if ([sectionViewModel respondsToSelector:@selector(headerViewModel)]) {
...
- (void)addCellViewModel:(__kindof id<FRTableItemViewModelProtocol>)cellViewModel
    ...
    FRTableSectionViewModel *section = self.sections[sectionIndex];
...
- (void)insertCellViewModel:(__kindof id<FRTableItemViewModelProtocol>)cellViewModel
    ...
    if (sectionIndex < self.sections.count) {
        for (FRTableSectionViewModel *section in self.sections) {
            for (id<FRTableItemViewModelProtocol> currentCellViewModel in section.mutableRows) {
...
- (void)addSectionViewModel:(id<FRTableSectionViewModelProtocol>)sectionViewModel
    ...
    FRTableSectionViewModel *section = sectionViewModel;
...
- (NSMutableArray<__kindof id<FRTableSectionViewModelProtocol>> *)sections {
    if (!_sections) {
...
@interface FRTableViewDelegate ()

@property (nullable, nonatomic, weak, readwrite) __kindof FRTableViewModel *viewModel;

@end

@implementation FRTableViewDelegate
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<FRTableItemViewModelProtocol> cellViewModel = [self.viewModel cellViewModelFromIndexPath:indexPath];
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 9.0) {
        ...
        id<FRTableViewItemProtocol> cell =
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    ...
    id<FRTableSectionViewModelProtocol> sectionViewModel = self.viewModel.sections[section];
    if ([sectionViewModel respondsToSelector:@selector(headerViewModel)]) {
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    id<FRTableSectionViewModelProtocol> sectionViewModel = self.viewModel.sections[section];
    if ([sectionViewModel respondsToSelector:@selector(headerViewModel)]) {
        id<FRTableItemViewModelProtocol> headerViewModel = sectionViewModel.headerViewModel;
        return (UIView *)[self.viewModel.tableViewFactory headerWithViewModel:headerViewModel tableView:tableView];
    } else if ([sectionViewModel respondsToSelector:@selector(headerTitle)]) {
        FRTableViewHeader *defaultHeader =
        [tableView dequeueReusableHeaderFooterViewWithIdentifier:kDefaultHeaderReuseIdentifier];
        if (!defaultHeader) {
            defaultHeader = [[FRTableViewHeader alloc] initWithReuseIdentifier:kDefaultHeaderReuseIdentifier];
        }
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    id<FRTableSectionViewModelProtocol> sectionViewModel = self.viewModel.sections[section];
    if ([sectionViewModel respondsToSelector:@selector(footerViewModel)]) {
        id<FRTableItemViewModelProtocol> footerViewModel = sectionViewModel.footerViewModel;
        return (UIView *)[self.viewModel.tableViewFactory footerWithViewModel:footerViewModel tableView:tableView];
    } else if ([sectionViewModel respondsToSelector:@selector(footerTitle)]) {
        FRTableViewFooter *defaultFooter =
        [tableView dequeueReusableHeaderFooterViewWithIdentifier:kDefaultFooterReuseIdentifier];
        if (!defaultFooter) {
            defaultFooter = [[FRTableViewFooter alloc] initWithReuseIdentifier:kDefaultFooterReuseIdentifier];
        }
...
@interface FRTableViewDataSource ()

@property (nullable, nonatomic, weak, readwrite) __kindof FRTableViewModel *viewModel;

@end

@implementation FRTableViewDataSource
